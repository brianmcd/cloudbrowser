...
(function() {
  var AUTH_FAIL, CBAuthentication, EMAIL_EMPTY, EMAIL_INVALID, EMAIL_IN_USE, EMAIL_RE, PASSWORD_EMPTY, RESET_SUCCESS, User, appConfig, auth, curVB, googleStrategy, localStrategy;
  ...
  CBAuthentication.controller("LoginCtrl", function($scope) {
    ...
    return $scope.sendResetLink = function() {
      ...
      if (!($scope.email && EMAIL_RE.test($scope.email.toUpperCase()))) {
        ...
        return auth.sendResetLink(user, function(err, success) {
          return $scope.safeApply(function() {
            if (err) {

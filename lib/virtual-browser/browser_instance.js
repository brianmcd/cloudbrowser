...
var BrowserInstance = function (advice) {
    ...
    // TODO: Should I merge the stuff in DOM into Zombie, and then just have this.browser?
    /*
    self.dom.browser.on('loaded', function (browser) {
        console.log("The page changed!");
        //TODO resync
    });
    */
    self.__defineGetter__('window', function () {
        return self.dom.browser.window;
    });
    self.__defineGetter__('document', function () {
        return self.dom.browser.document;
    });
    // We want each client to be able to call the same API instance.
    self.ClientAPI = new BrowserInstanceClientAPI(self.dom.browser);
};

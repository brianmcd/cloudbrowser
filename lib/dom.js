...
var DOM = module.exports = function (advice) {
    //TODO: aliasing here with with every instance, which means our advice is aliasing.
    //      can we add our advice in such a way that it's ok to share?
    //          that would require each node to have a link to its browserinstance, so we can use that envIDTable
    //TODO: This is obviously aliasing if jsdom.jsdom(html) is returning a dom with advice, cause i forgot to pass this in.
...
DOM.prototype.loadHTML = function (html, callback) {
    ...
    this.document = jsdom.jsdom(html, this.dom);

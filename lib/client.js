    ...
    execute : function (inst) {
        switch (opCodeStr[inst.opcode]) {
            case 'CREATE_NODE':
                ...
                if (inst.nodeType != 3) {
                    elem.setAttribute("class", inst.__envID);
                }
                if (inst.targetID == 'document') {
                    var parent = this.document;
                } else {
                    var parent = this.document.getElementsByClassName(inst.targetID)[0];
                    //TODO: augment the document with an implementation if it doesn't exist
                    console.log('Relying on non-standard getElementsByClassName.');
                }

...
module.exports = Class.create({
    ...
    initialize : function (envChoice) {
        ...
        this.loadHTML = function (html, callback) {
            that.pageLoaded = true;
            oldLoad.call(that, html, callback);
        };
    },
        ...
        if (url.hostname) {
            ...
            request({uri: url}, function (err, request, body) {
                if (err) {
                    ...
                    that.loadHTML(body, opts.success);
                }
            });
        } else {
        ...
        if (path == "" || path == undefined) {
            ...
            fs.readFile(path, 'utf8', function (err, data) {
                if (err) {
                    ...
                    data = data.replace(/\n$/, '');
                    data = data.replace(/\r$/, '');
                    console.log('Sending to loadHTML: ' + data);
                    that.loadHTML(data, opts.success);
                }
            });
        }
        return this;

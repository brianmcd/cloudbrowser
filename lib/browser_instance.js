...
module.exports = Class.create({
    ...
    dispatchEvent : function (eventInfo) {
        ...
        console.log("Dispatching event: " + ev.type + " on " +
                    target.__envID + '(' + target.nodeType + ':' + 
                    target.nodeName + ')');
        ...
        //    group.forEach(function (eventType) {
            MouseEvents.forEach(function (eventType) {
                self.document.addEventListener(eventType, function (event) {
                    ...
                    var target = self.envIDTable[event.target.__envID];
                    if (target && target.tagName && 
                        (target.tagName.toLowerCase() == 'a') && target.href) {
                        self.linkClickHandler(event, target);
                    }
                }, true /* capturing */);
            });
       // });
    ...
    linkClickHandler : function (event, target) {
        var self = this;
        ...
        if (target.href.match(/^\//)) {
            ...
            var filename = path.join('/home/brianmcd/projects/vt-node-lib/examples/test-server', target.href); // TODO: This is just a hack for testing
            console.log('File to load: ' + filename);
            self.load(filename, function () {
                console.log('New page loaded');
                self.client.send(JSON.stringify(self.resync())); //TODO: move this outside of callback.
            });

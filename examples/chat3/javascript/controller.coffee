...
Chat3.controller "ChatCtrl", ($scope) ->
    ...
    addRoom = (room, roomList, setupListeners) ->
        if not findRoom(room.name, roomList) then roomList.push(room)
        if setupListeners then room.on "NewMessage", (message) ->
            $scope.safeApply()
    ...
    if chatManager then for room in chatManager.getAllRooms()
        if not findRoom(room.name, $scope.joinedRooms)
            addRoom(room, $scope.otherRooms, false)

    $scope.user.on "JoinedRoom", (room) ->
        $scope.safeApply ->
    ...
    $scope.user.on "LeftRoom", (name) ->
        $scope.safeApply ->
            ...
            lastActiveRoom = null
            $scope.activeRoom = getLastActiveRoom()

    for room in $scope.user.getAllRooms()
        addRoom(room, $scope.joinedRooms, true)
        $scope.activate(room)
